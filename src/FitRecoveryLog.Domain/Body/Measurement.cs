...
public sealed class Measurement : AggregateRoot
{
    ...
    public static Measurement Rehydrate(Guid id, DateOnly date, double? weight, double? waist, double? chest,
        double? hips, double? arms, double? thighs, double? shoulders, double? calves, double? bodyFat, double? muscle,
        double? visceral, double? water, int? bmr, int? metabolicAge, string? notes)
    {
        var m = new Measurement(id, date);
        // Assign directly — reconstructing persisted state must not raise a domain event.
        m.Assign(weight, waist, chest, hips, arms, thighs, shoulders, calves, bodyFat, muscle, visceral, water, bmr, metabolicAge, notes);
        return m;
    }
    ...
    {
        if (new double?[] { weight, waist, chest, hips, arms, thighs, shoulders, calves, bodyFat, muscle, visceral, water }.Any(v => v < 0)
            || bmr < 0 || metabolicAge < 0)
            throw new ArgumentOutOfRangeException(nameof(weight), "Measurement values cannot be negative.");

        Assign(weight, waist, chest, hips, arms, thighs, shoulders, calves, bodyFat, muscle, visceral, water, bmr, metabolicAge, notes);
        ...
        int? bmr, int? metabolicAge, string? notes)
    {
        ...
        ArmsInches = arms; ThighsInches = thighs; ShouldersInches = shoulders; CalvesInches = calves;
