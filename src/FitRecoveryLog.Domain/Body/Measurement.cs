...
public sealed class Measurement : AggregateRoot
{
    ...
    public double? ForearmsInches { get; private set; }
    public double? ThighsInches { get; private set; }
    public double? ShouldersInches { get; private set; }
    public double? CalvesInches { get; private set; }
    ...
    public static Measurement Rehydrate(Guid id, DateOnly date, double? weight, double? waist, double? chest,
        double? hips, double? arms, double? forearms, double? thighs, double? shoulders, double? calves, double? bodyFat,
        double? muscle, double? visceral, double? water, int? bmr, int? metabolicAge, string? notes)
    {
        var m = new Measurement(id, date);
        // Assign directly — reconstructing persisted state must not raise a domain event.
        m.Assign(weight, waist, chest, hips, arms, forearms, thighs, shoulders, calves, bodyFat, muscle, visceral, water, bmr, metabolicAge, notes);
        return m;
    }
    ...
    public void Update(double? weight, double? waist, double? chest, double? hips, double? arms, double? forearms, double? thighs,
        double? shoulders, double? calves, double? bodyFat, double? muscle, double? visceral, double? water,
        int? bmr, int? metabolicAge, string? notes)
    {
        if (new double?[] { weight, waist, chest, hips, arms, forearms, thighs, shoulders, calves, bodyFat, muscle, visceral, water }.Any(v => v < 0)
            || bmr < 0 || metabolicAge < 0)
            throw new ArgumentOutOfRangeException(nameof(weight), "Measurement values cannot be negative.");

        Assign(weight, waist, chest, hips, arms, forearms, thighs, shoulders, calves, bodyFat, muscle, visceral, water, bmr, metabolicAge, notes);

        if (WeightLbs is not null || WaistInches is not null)
            Raise(new MeasurementRecorded(Id, Date, WeightLbs, WaistInches));
    }

    private void Assign(double? weight, double? waist, double? chest, double? hips, double? arms, double? forearms, double? thighs,
        double? shoulders, double? calves, double? bodyFat, double? muscle, double? visceral, double? water,
        int? bmr, int? metabolicAge, string? notes)
    {
        WeightLbs = weight; WaistInches = waist; ChestInches = chest; HipsInches = hips;
        ArmsInches = arms; ForearmsInches = forearms; ThighsInches = thighs; ShouldersInches = shoulders; CalvesInches = calves;
        BodyFatPercent = bodyFat; MuscleMassLbs = muscle;
